...
class Sell {
  ...
  Future<bool> createApiSell({sellId, bool? syncAll}) async {
    ...
    sales.forEach((element) async {
      ...
      List<Map<String, dynamic>> sale = [
        {
          ...
          'sub_status': (element['is_quotation'].toString() == '1')
              ? 'quotation'
              : null,
          'tax_rate_id': (element['tax_rate_id'] == 0)
              ? null
              : element['tax_rate_id'],
      ...
      List paymentDetail = await PaymentDatabase().getPaymentLineByReturnValue(
        element['id'],
        1,
      );
      //set change returnId for updating a sell in API
      var returnId = (paymentDetail.isNotEmpty)
          ? paymentDetail[0]['payment_id']
          : null;
      ...
      Map<String, dynamic> editedSale = {
        ...
        'products': products,
        'payments': await PaymentDatabase().getPaymentLineByReturnValue(
          element['id'],
          0,
        ),
      };
      if (element['is_synced'] == 0) {
        if (element['transaction_id'] != null) {
          ...
          Map<String, dynamic> updatedResult = await SellApi().update(
  ...
  void makePayment(List payments, int sellId) {
  ...
  //get unit_price
  Future<double> getUnitPrice(unitPrice, taxId) async {
    double price = 0.00;
...
* */
  ...
  Future<void> addToCart(product, sellId) async {
  ...
  Future<String> cartItemCount({isCompleted, sellId}) async {
    return await SellDatabase().countSellLines(
  ...
  Map<String, dynamic> createSellMap(Map sell, change, pending) {
    Map<String, dynamic> sale = {
      ...
      'discount_amount': (sell['discount_amount'] != null)
