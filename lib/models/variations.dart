...
import '../data/services/variations_service.dart';
...
class Variations {
  ...
  //get variations from API directly
  Future<List<Map<String, dynamic>>> getFromApi({
    brandId,
    categoryId,
    subCategoryId,
    searchTerm,
    ...
    var response = await VariationsApi().get(url);
    List products = response['products'] ?? [];
    List<Map<String, dynamic>> result = [];

    for (var variation in products) {
      double stockAvailable = 0;
      if (variation['variation_location_details'] != null &&
          variation['variation_location_details'].length > 0) {
        for (var loc in variation['variation_location_details']) {
          if (loc['location_id'].toString() == locationId.toString()) {
            stockAvailable =
                double.tryParse(loc['qty_available'].toString()) ?? 0;
          }
        }
      }
      ...
      result.add({
        ...
        'enable_stock': variation['enable_stock'],
        'tax_id': variation['tax_id'],
        'sell_price_inc_tax': variation['sell_price_inc_tax'],
        'product_image_url': variation['product_image_url'],
        'selling_price_group': variation['selling_price_group'] != null
            ? jsonEncode(
                variation['selling_price_group']
                    .map(
                      (group) => {
                        'key': group['price_group_id'],
                        'value': group['price_inc_tax'],
