...
class _SalesState extends State<Sales> {
  ...
  getPermission() async {
    ...
    if (await Helper().getPermission("direct_sell.view")) {
      ...
      url = Api().baseUrl +
          Api().apiUrl +
          "/sell?order_by_date=desc&user_id=${Config.userId}";
      if (paymentStatuses.length < 2) {
        paymentStatuses.addAll(['paid', 'due', 'partial', 'overdue']);
        selectedPaymentStatus = 'all';
      }
              ...
              Row(
                ...
                children: <Widget>[
                  Visibility(
                    ...
                    child: Row(
                      children: [
                        // (canEditSell) ? IconButton(icon: Icon(MdiIcons.fileDocumentEditOutline, color: themeData.colorScheme.onSurface,), onPressed: () {Navigator.push(context, '/cart', arguments: Helper().argument(locId: sellList[index]['location_id'], sellId: sellList[index]['id'], isQuotation: sellList[index]['is_quotation']));}) : Container(),
                        (canDeleteSell)
                            ? IconButton(
                                icon: Icon(
                                  MdiIcons.deleteOutline,
